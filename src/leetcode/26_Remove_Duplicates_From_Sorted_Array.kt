 ...
 *
 * Do not allocate extra space for another array, you must do this by modifying the input array [in-place](https://en.wikipedia.org/wiki/In-place_algorithm)
 ...
 * modified to 0, 1, 2, 3, and 4 respectively. It doesn't matter what values are set beyond the returned length.
 * ```
 *
 * ## Constraints:
 * ```
...
fun main() {
  check(`26_Remove_Duplicates_From_Sorted_Array`.Solution().removeDuplicates(intArrayOf(1, 1, 2)) == 2)
  check(`26_Remove_Duplicates_From_Sorted_Array`.Solution().removeDuplicates(intArrayOf(1, 2, 2)) == 2)
  check(`26_Remove_Duplicates_From_Sorted_Array`.Solution().removeDuplicates(intArrayOf(0, 0, 1, 1, 1, 2, 2, 3, 3, 4)) == 5)
